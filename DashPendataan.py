...
with st.sidebar:
    ...
    if len(tanggal_range) != 2:
        tanggal_awal = pendataan["Tanggal Diterima"].min().date()
        tanggal_akhir = pendataan["Tanggal Diterima"].max().date()
    
    else:
        tanggal_awal, tanggal_akhir = tanggal_range
    
    # Filter data
    pendataanFiltered = pendataan[
...
def show_chart(data_primer, data_sekunder, label_primer, label_sekunder, warna):
    ...
    fig.update_layout(
        ...
        showlegend = False,
        annotations = [dict(
            x = pendataan["idbs"].count()/2,
            y = 0,
            text = f"{data_primer} ({100*(data_primer/pendataan["idbs"].count()):.2f}%)",
            showarrow = False,
...
# Hitung data
jumlah_masuk = pendataanFiltered["Tanggal Diterima"].notna().sum()
jumlah_belum_masuk = len(pendataan) - jumlah_masuk

jumlah_mulai_entri = pendataanFiltered["Tanggal Entri"].notna().sum()
